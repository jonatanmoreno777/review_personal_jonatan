...
# Load the shapefile
c_shp <- st_read("SHP/cachi_wg84_R.shp")
...
lat_range <- seq(-13.54, -12.81, by = 0.10)  # Ajuste a los límites de latitud cachi
lon_range <- seq(-74.76, -73.92, by = 0.10)

lat_range <- seq(-19.35, 2.32, by = 0.10)  # Ajuste a los límites de latitud of Peru
lon_range <- seq(-82.48, -66.67, by = 0.10)
...
write.csv(points_within_pd_df, "CDBC.csv", row.names = FALSE)  #okkkk
save(points_within_pd_df, file = "grid2.rda")
...
write.csv(tDat2, "CDBC-choccoro-3.csv", row.names = F) 
# Lee el archivo CSV y carga en un data frame
df <- read.csv("bilinealok2.csv")
save(df, file = "archivo.rda")
#okkkk
...
write.csv(final_output, "CDBC-choccoro-BC.csv", row.names = FALSE, col.names = FALSE)
...
lat_min <- -18.39  # Latitud mínima
lat_max <- -0.04   # Latitud máxima
lon_min <- -81.39  # Longitud mínima
lon_max <- -68.65  # Longitud máxima
...
shapefile_path <- "SHP/DEPARTAMENTOS.shp"  # Cambia esto a la ruta de tu shapefile
