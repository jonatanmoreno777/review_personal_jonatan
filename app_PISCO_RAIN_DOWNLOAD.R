...
data <- st_read('D:/Paper_Climate/Data/siguiente paper/App/app2/Basins/Climate change/Climate_change_CanGLWS.shp')
...
data <- st_read('D:/Paper_Climate/Data/siguiente paper/App/app2/Basins/Historical Observed/Historicaly_observed_CanGLWS.shp')
st_geometry(data) <- NULL
...
labels <- sprintf(
  ...
  boundary$LAKEBASIN, boundary$AREA #boundary$LAKEBASIN, boundary$AREA
) %>% lapply(htmltools::HTML)

# Definir los colores de los polígonos
boundary$col <- c('red', 'blue', 'green', 'yellow')  #boundary$col <- c('red', 'blue', 'green', 'yellow')
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({    #RANDERIZAR EL MAPA
    basemap
  })
